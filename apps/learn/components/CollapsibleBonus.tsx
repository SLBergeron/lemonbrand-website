...
import { ChevronDown, Gift, Copy, Check } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

interface CollapsibleBonusProps {
  title: string;
...
// Copiable prompt block for use within bonus content
...
export function CollapsibleBonus({
  title,
  children,
  defaultOpen = false,
  className,
}: CollapsibleBonusProps) {
  const [isOpen, setIsOpen] = useState(defaultOpen);

  return (
    <div className={cn("relative rounded-2xl overflow-hidden my-8", className)}>
      {/* Outer glow effect */}
      ...
      <div className="relative bg-gradient-to-br from-card/95 via-card to-accent/[0.02] border border-accent/20 rounded-2xl overflow-hidden backdrop-blur-sm">
        ...
        <button
          ...
          className="relative w-full px-6 py-5 flex items-center justify-between hover:bg-accent/[0.03] transition-colors duration-300 text-left"
        >
          <div className="flex items-center gap-4">
            {/* Icon with subtle pulse when open */}
